...
# `pass-cli item update --field x=y` (used to add secrets to an existing
# item after its initial creation, e.g. scripts/pass-add-*-secret.sh)
# writes into a separate top-level `extra_fields` array, not into any
# section — confirmed live, no CLI flag targets a section instead. Fields
# added this way were silently invisible to every app until this line was
# added, since only `content.content.Custom.sections` was read before.
